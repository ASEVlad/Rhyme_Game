...
// --- variant A: PROD prompt for Ukrainian (mirrors deployed lib/languages.ts as of 2026-05-17) ---
const promptProd = (theme) => [
  ...
  'У кожній групі — 3–4 слова. Перевага — простим словам, які впізнає підліток або початківець.',
...
// --- variant: v7 — v6 with abstract example + explicit count for POS-mix & length-mix groups ---
const promptV7 = (theme) => [
  `Тема для натхнення: "${theme}". Це лише поштовх, не клітка.`,
  `Згенеруй ${COUNT} груп українських слів, які римуються в межах кожної групи.`,
  'ФОРМАТ КОЖНОЇ ГРУПИ:',
  '• "ending" — буквальне закінчення, яким завершується КОЖНЕ слово групи. Формат: дефіс + 2–5 літер. Без коментарів, дужок, слешів. Якщо не всі слова групи мають однакове закінчення — розділи на дві групи.',
  '• Кожне слово — ОДНЕ існуюче українське слово у звичній формі. Без пробілів, прийменників, новотворів, латиниці.',
  '• Усі слова в групі мають справді римуватися (однаковий наголошений голосний і звуки після нього).',
  '• Не повторюй слово в межах групи.',
  'ПРИКЛАД ПРАВИЛЬНОЇ ГРУПИ: { "ending": "-ить", "words": ["летить", "горить", "болить", "кричить"] }.',
  'ПРИКЛАД НЕПРАВИЛЬНОЇ: { "ending": "-ають", "words": ["співають", "гуляють", "гризуть", "кричать"] } — різні закінчення.',

  'ВНУТРІШНЯ РІЗНОМАНІТНІСТЬ ГРУПИ (КРИТИЧНО):',
  '• За замовчуванням слова в групі мають той самий звук — це робить їх схожими і за частиною мови, і за довжиною. Це нудно для римера.',
  '• ОБОВʼЯЗКОВО серед твоїх ' + COUNT + ' груп:',
  '  — щонайменше 2 групи мають МІКС ЧАСТИН МОВИ всередині (наприклад прикметник + іменник, або іменник + прислівник в одній групі). Це означає: підбираєш закінчення, яке природно дозволяє різні частини мови, і кладеш у групу слова РІЗНИХ частин мови.',
  '  — щонайменше 2 групи мають МІКС ДОВЖИНИ слів (короткі 1–2 склади поряд з довшими 3–4 складами).',
  '  — добре, якщо ці вимоги поєднуються (одна група і POS-мікс, і довжину-мікс).',
  '• Якщо закінчення фізично допускає лише одну частину мови (наприклад "-ують" чи "-ого") — це нормальна моно-група. Але серед 10 закінчень знайди такі, які дозволяють мікс, і використай їх.',

  'ЛЕКСИЧНА РІЗНОМАНІТНІСТЬ МІЖ ГРУПАМИ:',
  '• Кожна група — з різного семантичного гнізда. Не створюй кілька груп навколо одного кореня.',
  '• ЗАБОРОНЕНО: одна група з "магічна, музична", інша з "магічний, музичний", третя з "магічно, музично" — це той самий лексикон у різних формах.',
  '• Жодне слово не повторюється у двох групах.',

  `РІЗНОМАНІТТЯ ЗА ЧАСТИНАМИ МОВИ НА РІВНІ ${COUNT} ЗАКІНЧЕНЬ — набір має включати:`,
  '• щонайменше 2 прикметникові закінчення (наприклад -ий, -на, -ого, -ій, -ний),',
  '• щонайменше 1 прислівникове (наприклад -о, -е: тихо, легко, разом, потім),',
  '• щонайменше 1 дієслівне в особовій формі (-ить, -ать, -ять, -ують),',
  "• щонайменше 1 'якірне' — таке, де живуть конкретні образи (людина, час доби, частина тіла, погода, місце).",

  'СЕМАНТИКА: тема — натхнення для приблизно 30–40% груп. Інші мають вести римера в інші напрямки.',
  'У кожній групі — 3–4 слова. Перевага — простим словам, які впізнає підліток або початківець.',
  'Уникай рідкісних, архаїчних або вульгарних слів.',
  'Виведи результат через інструмент rhyme_groups.',
].join(' ');

// --- variant: v6 (kept for reference) ---
const promptV6 = (theme) => [
  `Тема для натхнення: "${theme}". Це лише поштовх, не клітка.`,
  `Згенеруй ${COUNT} груп українських слів, які римуються в межах кожної групи.`,
  'ФОРМАТ КОЖНОЇ ГРУПИ:',
  '• "ending" — буквальне закінчення, яким завершується КОЖНЕ слово групи. Формат: дефіс + 2–5 літер ("-ить", "-ова", "-шка"). Без коментарів, дужок, слешів, альтернатив. Якщо не всі слова групи мають однакове закінчення — розділи на дві групи.',
  ...
  '• Не повторюй слово в межах групи.',
  'ПРИКЛАД ПРАВИЛЬНОЇ ГРУПИ: { "ending": "-ить", "words": ["летить", "горить", "болить", "кричить"] }.',
  'ПРИКЛАД НЕПРАВИЛЬНОЇ: { "ending": "-ають", "words": ["співають", "гуляють", "гризуть", "кричать"] } — так не роби, різні закінчення.',

  'ВНУТРІШНЯ РІЗНОМАНІТНІСТЬ ГРУПИ (важливо):',
  '• Не всі групи мають бути моно-частиномовними чи одно-довгими. Там, де закінчення допускає, мікшуй у межах ОДНІЄЇ групи різні ЧАСТИНИ МОВИ і різну ДОВЖИНУ слів. Це робить рими корисними для римера.',
  '• ПРИКЛАД мікс частин мови: { "ending": "-ова", "words": ["нова", "розмова", "готова", "голова"] } — прикметники "нова, готова" разом з іменниками "розмова, голова".',
  '• ПРИКЛАД мікс довжин: { "ending": "-ого", "words": ["мого", "нового", "сміливого", "казкового"] } — від 2 до 4 складів в одній групі.',
  '• Якщо закінчення фізично допускає лише одну частину мови (наприклад "-ують" — тільки дієслова) або однакову довжину — це ок. Але там, де можна, мікс кращий за моноблок.',
  ...
  'СЕМАНТИКА: тема — натхнення для приблизно 30–40% груп. Інші мають вести римера в інші напрямки. Не зациклюйся на одному семантичному полі.',
  'У кожній групі — 3–4 слова. Перевага — простим словам, які впізнає підліток або початківець.',
  'Уникай рідкісних, архаїчних або вульгарних слів.',
  'Виведи результат через інструмент rhyme_groups.',
].join(' ');
...
const VARIANTS = [
  { name: 'prod', build: promptProd },
  { name: 'v7',   build: promptV7 },
];
